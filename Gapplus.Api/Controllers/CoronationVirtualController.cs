...
namespace BarcodeGenerator.Controllers
{
    ...
    public class CoronationVirtualController : ControllerBase
    {
        private readonly ITempDataManager _tempDataManager;
        private readonly IViewBagManager _viewBagManager;

        public CoronationVirtualController(UsersContext context, ITempDataManager tempDataManager, IViewBagManager viewBagManager)
        {
                ...
                else
                {
                    if (model.shareholder != null)
                    {
                        ...
                        if (UniqueAGMId != -1)
                        {
                            ...
                            var checkToStartAdmittance = db.Present.Any(p => p.AGMID == UniqueAGMId && p.ShareholderNum == model.shareholder.ShareholderNum);
                            // ViewBag.startadmittance =checkToStartAdmittance;
                            _viewBagManager.SetValue("startadmittance", checkToStartAdmittance);
