...
namespace BarcodeGenerator.Controllers
{
    ...
    public class AccreditationController : ControllerBase
    {
        ...
        public AccreditationController(
         ...
         ITempDataManager tempDataManager,
          IViewBagManager viewBagManager,
        ...
        private async Task<AccreditationResponse> IndexAsync()
        {
            try
            {
                ...
                return await Task.FromResult<AccreditationResponse>(model);

            }
            catch (Exception e)
            {
                return await Task.FromResult<AccreditationResponse>(new AccreditationResponse());
            }
        ...
        [HttpGet]
        public async Task<ActionResult> AGMStageLogin(string companyinfo)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<TimeSpan>> aElapsedTimeEventLoadingAsync(string company)
        {
            ...
            if (!string.IsNullOrEmpty(company))
            {
                ...
                // return Task.FromResult<TimeSpan>(time);
                // return Ok(Task.FromResult<TimeSpan>(time));
                var t = Task.FromResult<TimeSpan>(time);
                return Ok(t);
  ...
  private Task<string> EventResolutionAsync(string company)
                ...
                if (model.ResourceType == "Facilitator")
                {
                    if (model.facilitator != null)
                    {
                        ...
                        if (UniqueAGMId != -1)
                        {
                            ...
                            _viewBagManager.SetValue("facilitatoremail", model.facilitator.emailAddress);
                            _viewBagManager.SetValue("AGMTitle", model.AGMTitle.ToUpper());
                            _viewBagManager.SetValue("resourcetype", model.ResourceType);
                            _viewBagManager.SetValue("startadmittance", false);
                ...
                else
                {
                    if (model.shareholder != null)
                    {
                        ...
                        if (UniqueAGMId != -1)
                        {
                            ...
                            _viewBagManager.SetValue("shareholderemail", model.shareholder.emailAddress);
                            _viewBagManager.SetValue("AGMTitle", model.AGMTitle.ToUpper());
                            _viewBagManager.SetValue("resourcetype", model.ResourceType);
                            ...
                            var startAdmittance = db.Present.Any(p => p.AGMID == UniqueAGMId && p.ShareholderNum == model.shareholder.ShareholderNum);
                            // ViewBag.startadmittance =startAdmittance;
                            _viewBagManager.SetValue("startadmittance", startAdmittance);
                            if (model.ResponseCode == "200")
                            {
                                var oldsessionversion = await WebSessionManager.CreateUserLoginHistory(model.shareholder.Company, model.shareholder.emailAddress, sessionid);
                            ...
                            _viewBagManager.SetValue("userProxyStatus", model.UserProxyStatus);
                            _viewBagManager.SetValue("pollStatus", model.UserPollStatus);
        ...
        [HttpPost]
        public async Task<ActionResult<APIMessageLog>> AGMQuestionAsync(string question, string shareholdernum, string company)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(question) || string.IsNullOrWhiteSpace(shareholdernum))
                {
                    ...
                    var r = await Task.FromResult<APIMessageLog>(MessageLog);
                    return Ok(r);
                }
                ...
                else
                {
                    ...
                    var r = Task.FromResult<APIMessageLog>(MessageLog);
                    return Ok(r);
...
[HttpGet]
        ...
        protected void Application_BeginRequest()
        {
            ...
            _cacheService.DisableCache(Response);
        }
        ...
        public async Task<ActionResult> AccreditationConfirmation(string query)
        {
            ...
            SessionManager.SetSessionData("Authorize", model);
            if (model.ResponseCode == "206")
            {
                // return RedirectToAction("AGMStageLogin");
                // return await AGMStateLogin();
                return Ok();
            }
            else if (model.ResponseCode == "200")
            {
                ...
                var startAdmittance = db.Present.Any(p => p.AGMID == model.AGMID && p.ShareholderNum == model.shareholder.ShareholderNum);
        ...
        // [HttpPost]
        private async Task<APIMessageLog> AccreditationConfirmationAsync(string encryptedText, string sessionid)
        {
        ...
        public async Task<ActionResult> ExternalAdmission(string query)
        {
            ...
            SessionManager.SetSessionData("Authorize", response as APIMessageLog);
        ...
        // [HttpPost]
        private Task<APIMessageLog> ExternalAdmissionAsync(string encryptedText, string sessionid)
        {
            //var query = new VoteModel();
            string companyinfo;
            string emailAddress = "";
        ...
        public async Task<ActionResult> VotingResponse(VoteModel post)
        {
