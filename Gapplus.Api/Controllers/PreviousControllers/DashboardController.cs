...
namespace BarcodeGenerator.Controllers
{
    ...
    public class DashboardController : ControllerBase
    {
        ...
        public DashboardController(UsersContext context, IViewBagManager viewBagManager)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            // Check if the "rel" parameter exists in the query string
            if (HttpContext.Request.Query.ContainsKey("rel"))
            {
                // Retrieve the value of the "rel" parameter
                returnvalue = HttpContext.Request.Query["rel"].ToString();
            }

            // Trim the obtained query string value and assign it to ViewBag.value
            // ViewBag.value = returnvalue.Trim();
        ...
        private Task<ReportRefreshModelView> IndexAsync()
        {
        ...
        private Task<ResolutionListModel> ResolutionChartAsync(int id)
        {
            Double holding = 0;
            ...
            _viewBagManager.SetValue("ResolutionName", question.question);
            if (UniqueAGMId != -1)
            {
                var agmEventSetting = db.Settings.SingleOrDefault(s => s.AGMID == UniqueAGMId);
                ...
                _viewBagManager.SetValue("ResultForCount", model.ResultFor.Count());
                _viewBagManager.SetValue("ResultAgainstCount", model.ResultAgainst.Count());
                _viewBagManager.SetValue("ResultAbstainCount", model.ResultAbstain.Count());

                //Percentage voters 

                var TotalPresent = db.Present.Where(p => p.AGMID == UniqueAGMId && p.PermitPoll == 1).Count();
                var percentageFor = ((double)model.ResultFor.Count() / (double)TotalPresent) * 100;
                var percentageAgainst = ((double)model.ResultAgainst.Count() / (double)TotalPresent) * 100;
                var percentageAbstain = ((double)model.ResultAbstain.Count() / (double)TotalPresent) * 100;

                //   ViewBag.PercentageFor=percentageFor;
                //    ViewBag.PercentageAgainst=percentageAgainst;
                //           ViewBag.PercentageAbstain=percentageAbstain;

                _viewBagManager.SetValue("PercentageFor", percentageFor);
                _viewBagManager.SetValue("PercentageAgainst", percentageAgainst);
                _viewBagManager.SetValue("PercentageAbstain", percentageAbstain);
        ...
        private Task<ResolutionListModel> VotingBoardAsync()
        {
            ResolutionListModel model = new ResolutionListModel();
            var companyinfo = ua.GetUserCompanyInfo();
            var UniqueAGMId = ua.RetrieveAGMUniqueID();
        ...
        public async Task<ActionResult> PresentCount()
        {
            ...
            _viewBagManager.SetValue("response", response);
            // return PartialView();
            return Ok();
        }
