...
namespace BarcodeGenerator.Controllers
{
...

    {
        ...
        private readonly UserAdmin ua;
        private readonly TempDataManager _tempDataManager;


        public ResolutionController(UsersContext _db)
        {
            db = _db;
            ua = new UserAdmin(db);
        ...
        private Task<QuestionListViewModel> IndexAsync()
        {
        ...
        private Task<Result> splitpollAsync(PostResolution post)
        {
            var companyinfo = ua.GetUserCompanyInfo();
            var UniqueAGMId = ua.RetrieveAGMUniqueID();
            ...
            else
            {
                ...
                if (setting == null)
                {
                    ...
                    _tempDataManager.SetTempData("Message", "Please value for Total Holding");
                    return Task.FromResult<Result>(new Result());
                }
                var totalHolding = setting.ShareHolding;
                Double TotalShareholding = totalHolding;
        ...
        private Task<Result> pollAsync(PostResolution post)
        {
            ...
            if (checkresult != null)
            {
                ...
                catch (DbUpdateConcurrencyException)
                {
                    ...
                    _tempDataManager.SetTempData("Message", "Error Adding your poll Information");
...
[HttpGet("{id}")]
        ...
        private Task<ResultListViewModel> DetailsAsync(int id, int page = 1)
        {
            try
            {
                //var companyinfo = ua.GetUserCompanyInfo();
...
[HttpPost]
        ...
        private Task<ResultListViewModel> ResultDetailsIndexAsync(Result model)
        {
            try
            {
                //var companyinfo = ua.GetUserCompanyInfo();
    ...
    [HttpPost]
        ...
        private Task<ResultListViewModel> ResultDetailsAsync(Result model)
        {
            try
            {
                //var companyinfo = ua.GetUserCompanyInfo();
    ...
    [HttpPost]
        ...
        private Task<ResultListViewModel> PresentResultDetailsAsync(Result model)
        {
            try
            {
                //var companyinfo = ua.GetUserCompanyInfo();
...
[HttpGet("{id}")]
        ...
        private Task<ResolutionModel> RefreshVoteIndexAsync(int id)
        {
...
[HttpPost("{id}")]
        ...
        private Task<ResolutionModel> VoteIndexAsync(int id)
        {
...
[HttpPost("{id}")]        public async Task<ActionResult> PresentVoteIndex(int id)
        {
        ...
        private Task<ResolutionModel> PresentVoteIndexAsync(int id)
        {
            var shareholder = db.Present.Find(id);
            var companyinfo = ua.GetUserCompanyInfo();
            var UniqueAGMId = ua.RetrieveAGMUniqueID();
        ...
        private Task<Result> PresentVoteIndexAsync(PostResolution post)
        {
            ...
            else
            {
                ...
                catch (DbUpdateConcurrencyException)
                {
                    ...
                    // TempData["Message"] = "Error Adding your poll Information"; 
                    _tempDataManager.SetTempData("Message", "Error Adding your po ll Information");
